...
def get_cathegory(df_histogram):
    ...
    df["percent"] = df["percent"].map("{:.2f}%".format)  # Format as percentage
    print(df)
    return df
...
def calculate_values(df_copy):
    ...
    df_prob["probability"] = df_prob["probability"].map("{:.2f}%".format)
    #df_prob["percentage"] = df_prob["probability"].apply(lambda x: f"{x * 100:.2f}%")
...
def calculate_values_int_fp(df_copy):
    ...
    df_prob = pd.DataFrame(data_prob, columns=["InstrKind", "probability"])
    df_prob["probability"] = df_prob["probability"].map("{:.2f}%".format)  # Format as percentage
    df_count = pd.DataFrame(data, columns=["InstrKind", "count"])
    return df_prob, df_count
...
def plot_summary(df_sumary):
    # Convert probability column from percentage string to float
    df_sumary['probability'] = df_sumary['probability'].str.rstrip('%').astype(float) * 100
...
def compare_and_plot_totals(file1_path, file2_path):
    ...
    if total_count_file2 > total_count_file1:
        ...
        print(f"ratio = {total_count_file2/total_count_file1:.2f}")
    else:
        print(f"{file1_path} > {file2_path}")
        print(f"ratio = {total_count_file1/total_count_file2:.2f}")
